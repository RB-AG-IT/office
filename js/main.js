...
/**
 * Lädt alle Abrechnungsdaten für einen Botschafter
 * @param {string} userId - Botschafter UUID
 * @param {object} zeitraum - { von: Date, bis: Date }
 * @returns {Promise<object>} Abrechnungsdaten
 */
async function ladeAbrechnungsdaten(userId, zeitraum) {
    const supabase = window.parent?.supabaseClient || window.supabaseClient;
    if (!supabase) {
    ...
    try {
        // 1. Profil laden (Vorschuss-Anteil)
        const { data: profile, error: profileError } = await supabase
            .from('user_profiles')
            .select('advance_rate, reserve_rate')
            .eq('user_id', userId)
            .single();
        ...
        // 2. Alle Karrierestufen laden (für Zeitraum-Logik)
        const { data: careerEntries, error: roleError } = await supabase
            .from('user_roles')
        ...
        // Aktuelle Stufe mit Zeitraum-Logik ermitteln
        const career = getAktuelleKarriereStufe(careerEntries || []);

        // 3. Records für Zeitraum laden (nach Aufnahmedatum = start_date)
        const { data: records, error: recordsError } = await supabase
            .from('records')
            .select('id, yearly_amount, kw, year, record_status, start_date')
            .eq('werber_id', userId)
        ...
        let einheiten = 0;
        (records || []).forEach(r => {
            einheiten += berechneEinheiten(r.yearly_amount || 0);
        });

        // 7. Faktor ermitteln (aus Zeitraum-Logik)
        const faktor = career.factor;

        // 8. Provision berechnen
        const brutto = berechneBruttoProvision(einheiten, faktor);
        ...
        return {
            userId,
            zeitraum,
            einheiten: Math.round(einheiten * 100) / 100,
            faktor,
            karrierestufe: career.stufe,
            karriereAbgelaufen: career.isExpired, // true wenn Zeitraum abgelaufen aber Faktor weitergilt
            vorschussAnteil,
            brutto,
            vorschuss: aufteilung.vorschuss,
...
async function erstelleAbrechnung(data) {
    ...
    const invoice = {
        ...
        kw_start: getKW(data.zeitraum.von),
        kw_end: getKW(data.zeitraum.bis),
        year: data.year,
...
window.ladeAbrechnungsdaten = ladeAbrechnungsdaten;
...
// ============================================================================
// PDF-GENERATOR FÜR ABRECHNUNGEN
// ============================================================================

/**
 * Generiert eine Botschafter-Abrechnung als PDF
 * @param {object} data - Abrechnungsdaten
 * @returns {Promise<Blob>} PDF-Blob
 */
async function generateAbrechnungPDF(data) {
    // jsPDF laden (muss im HTML eingebunden sein)
    if (!window.jspdf) {
    ...
    // Farben
    const primaryColor = [45, 55, 72]; // Dunkelgrau
    const secondaryColor = [113, 128, 150]; // Mittelgrau

    // ========== HEADER ==========
    doc.setFontSize(24);
    doc.setTextColor(...primaryColor);
    doc.text('Gutschriftrechnung', margin, 25);

    // Rechnungsnummer und Datum (rechts)
    doc.setFontSize(10);
    doc.setTextColor(...secondaryColor);
    doc.text(`Rechnungsnr.: ${data.invoiceNumber}`, pageWidth - margin, 20, { align: 'right' });
    doc.text(`Datum: ${new Date().toLocaleDateString('de-DE')}`, pageWidth - margin, 26, { align: 'right' });
    ...
    doc.setFontSize(8);
    doc.setTextColor(...secondaryColor);
    doc.text('RB AG IT · Musterstraße 1 · 12345 Musterstadt', margin, 45);

    // ========== EMPFÄNGER ==========
    doc.setFontSize(11);
    doc.setTextColor(...primaryColor);
    doc.text(data.name, margin, 55);
    if (data.street && data.houseNumber) {
        doc.text(`${data.street} ${data.houseNumber}`, margin, 61);
    }
    if (data.zipCode && data.city) {
        doc.text(`${data.zipCode} ${data.city}`, margin, 67);
    }
    ...
    doc.setTextColor(...primaryColor);
    doc.text(`Karrierestufe: ${data.karrierestufe} (Faktor ${data.faktor})`, margin, 88);
    ...
    doc.setFontSize(8);
    doc.text(
        'RB AG IT · Musterstraße 1 · 12345 Musterstadt · info@rb-ag.de',
        pageWidth / 2,
...
/**
 * Lädt PDF in Supabase Storage hoch
 * @param {string} invoiceId - Invoice UUID
 * @param {Blob} pdfBlob - PDF-Blob
 * @returns {Promise<string>} Öffentliche URL
 */
async function uploadPdfToStorage(invoiceId, pdfBlob) {
    const supabase = window.parent?.supabaseClient || window.supabaseClient;
...
/**
 * Generiert und lädt PDF für eine Abrechnung hoch
 * @param {string} invoiceId - Invoice UUID
 * @returns {Promise<string>} PDF-URL
 */
async function generateAndUploadPdf(invoiceId) {
    const supabase = window.parent?.supabaseClient || window.supabaseClient;
    if (!supabase) throw new Error('Supabase nicht verfügbar');

    // Invoice mit User-Daten laden
    ...
    const pdfData = {
        ...
        city: profile.city,
        kw_start: invoice.kw_start,
        kw_end: invoice.kw_end,
        year: invoice.year,
        karrierestufe: calcData.karrierestufe || 'SMA',
        faktor: calcData.faktor || 5.0,
        einheiten: calcData.einheiten || 0,
        brutto: parseFloat(invoice.brutto_provision) || 0,
        vorschuss: parseFloat(invoice.vorschuss_betrag) || 0,
        stornorucklage: parseFloat(invoice.stornorucklage_betrag) || 0,
        vorschussAnteil: calcData.vorschuss_anteil || 75,
        abzuegeUnterkunft: parseFloat(invoice.abzuege_unterkunft) || 0,
    ...
    // PDF generieren
    const pdfBlob = await generateAbrechnungPDF(pdfData);

    // Upload
    const url = await uploadPdfToStorage(invoiceId, pdfBlob);

    return url;
}

/**
 * Öffnet PDF-Vorschau in neuem Tab
 * @param {object} data - Abrechnungsdaten
 */
async function previewAbrechnungPdf(data) {
    const pdfBlob = await generateAbrechnungPDF(data);
...
window.previewAbrechnungPdf = previewAbrechnungPdf;
