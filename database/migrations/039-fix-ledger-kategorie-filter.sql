-- Migration: Kategorie-Filter für Ledger-Verknüpfung
-- Datum: 2026-01-17
-- Beschreibung: Nur gewählte Kategorien werden mit Invoice verknüpft
...
BEGIN
    ...
    IF array_length(v_kategorien, 1) > 0 THEN
    ...
    -- 4. Euro-Ledger verknüpfen (immer alle im Zeitraum - Abzüge sind nicht kategorie-spezifisch)
    UPDATE euro_ledger
    SET invoice_id = new_invoice_id
    WHERE user_id = v_user_id
      AND invoice_id IS NULL
    ...
    -- 6. Stornorücklage Tracking (nur bei Vorschuss-Typ)
    IF v_invoice_type = 'vorschuss' AND COALESCE((input_data->>'gesamtStornorucklage')::NUMERIC, 0) > 0 THEN
        INSERT INTO stornorucklage_tracking (user_id, halbjahr, year, original_betrag, sperrfrist_bis, status)
        VALUES (
            v_user_id,
            CASE WHEN EXTRACT(MONTH FROM v_period_end) <= 6 THEN 1 ELSE 2 END,
            EXTRACT(YEAR FROM v_period_end)::INTEGER,
            (input_data->>'gesamtStornorucklage')::NUMERIC,
            (v_period_end + INTERVAL '12 months')::DATE,
            'gesperrt'
        )
        ON CONFLICT (user_id, halbjahr, year) DO UPDATE
        SET original_betrag = EXCLUDED.original_betrag,
            sperrfrist_bis = EXCLUDED.sperrfrist_bis;
    END IF;
