-- Migration 092: Record-IDs in calculation_data speichern (Getrennt-Modus)
-- Beim Zusammen-Modus werden die Positionen 1:1 gespeichert, recordIds sind dort automatisch enthalten.
-- Beim Getrennt-Modus muss calculation_data um sondierungRecordIds und regularRecordIds erweitert werden.
...
BEGIN
    ...
    IF v_rechnungsart = 'zusammen' THEN
        ...
        IF jsonb_array_length(COALESCE(input_data->'sondierungPositionen', '[]'::JSONB)) > 0 THEN
            ...
            -- Ledger-Einträge mit invoice_id aktualisieren (bei Sondierung)
        ...
        IF jsonb_array_length(COALESCE(input_data->'regularPositionen', '[]'::JSONB)) > 0 THEN
            ...
            -- Ledger-Einträge mit invoice_id aktualisieren (bei Regular)
            FOR v_zubuchung IN SELECT * FROM jsonb_array_elements(COALESCE(input_data->'zubuchungen', '[]'::JSONB))
            LOOP
                v_ledger_id := (v_zubuchung->>'ledgerId')::UUID;
                IF v_ledger_id IS NOT NULL THEN
        ...
        LOOP
            INSERT INTO invoices (
                ...
                jsonb_build_object(
                    ...
                    'sondierungRecordIds', COALESCE(v_rechnung->'sondierungRecordIds', '[]'::JSONB),
                    'regularRecordIds', COALESCE(v_rechnung->'regularRecordIds', '[]'::JSONB)
                )
            ) RETURNING id INTO v_new_invoice_id;
            ...
            -- Ledger-Einträge für dieses Gebiet mit invoice_id aktualisieren
            FOR v_zubuchung IN SELECT * FROM jsonb_array_elements(COALESCE(v_rechnung->'zubuchungen', '[]'::JSONB))
            LOOP
                v_ledger_id := (v_zubuchung->>'ledgerId')::UUID;
                IF v_ledger_id IS NOT NULL THEN
                    UPDATE drk_cost_ledger
...
COMMENT ON FUNCTION create_drk_invoice IS 'Erstellt DRK-Kundenrechnungen (Zusammen oder Getrennt) mit Ledger-Integration und Record-IDs';
