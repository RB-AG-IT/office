-- Migration 082: RPC create_drk_invoice erweitern für Ledger-Buchungen
-- Setzt invoice_id auf bestehende drk_cost_ledger Einträge (kein INSERT, nur UPDATE)
...
DECLARE
    ...
    v_record JSONB;
    v_zubuchung JSONB;  -- NEU für Ledger-Buchungen
BEGIN
    ...
    IF v_rechnungsart = 'zusammen' THEN
        ...
        IF jsonb_array_length(COALESCE(input_data->'sondierungPositionen', '[]'::JSONB)) > 0 THEN
            ...
            -- NEU: Ledger-Einträge mit invoice_id verknüpfen
            FOR v_zubuchung IN SELECT * FROM jsonb_array_elements(COALESCE(input_data->'zubuchungen', '[]'::JSONB))
            LOOP
                IF COALESCE((v_zubuchung->>'checked')::BOOLEAN, true) = true AND v_zubuchung->>'id' IS NOT NULL THEN
                    UPDATE drk_cost_ledger
                    SET invoice_id = v_new_invoice_id
                    WHERE id = (v_zubuchung->>'id')::UUID
    ...
    ELSE
        ...
        LOOP
            ...
            -- NEU: Ledger-Einträge für dieses Gebiet mit invoice_id verknüpfen
...
COMMENT ON FUNCTION create_drk_invoice IS 'Erstellt DRK-Kundenrechnungen und verknüpft Ledger-Einträge (Zusammen oder Getrennt)';
