-- ============================================================================
-- Migration 029: TC bekommt keine Provision für eigene Records
-- ============================================================================
--
-- Regel: teamleitung-Eintrag nur wenn teamchef_id ≠ werber_id
-- Bei TC-Wechsel: Neuer TC bekommt nur Provision wenn er ≠ Werber
--
-- Erstellt: 11.01.2026
-- ============================================================================

-- ============================================================================
-- 1. TRIGGER: Record INSERT (TC ≠ Werber Prüfung)
-- ============================================================================
...
-- ============================================================================
-- 2. TRIGGER: Record UPDATE (TC ≠ Werber Prüfung bei TC-Wechsel)
-- ============================================================================
...
BEGIN
    ...
    IF OLD.teamchef_id IS DISTINCT FROM NEW.teamchef_id THEN
        -- Alter TC: Storno (immer, auch wenn alter TC = Werber war - für Korrektur)
        ...
        -- Neuer TC: Buchung NUR wenn TC ≠ Werber
        IF NEW.teamchef_id IS NOT NULL AND NEW.teamchef_id IS DISTINCT FROM NEW.werber_id THEN
            INSERT INTO provisions_ledger (user_id, record_id, kategorie, typ, einheiten, kw, year, referenz_datum, beschreibung, campaign_id, campaign_area_id, customer_id)
    ...
    IF OLD.quality_id IS DISTINCT FROM NEW.quality_id THEN
        ...
        -- Neuer Quality: Buchung NUR wenn Quality ≠ Werber
        IF NEW.quality_id IS NOT NULL AND NEW.quality_id IS DISTINCT FROM NEW.werber_id THEN
            INSERT INTO provisions_ledger (user_id, record_id, kategorie, typ, einheiten, kw, year, referenz_datum, beschreibung, campaign_id, campaign_area_id, customer_id)
...
-- ============================================================================
-- 3. BESTANDSDATEN: Falsche teamleitung-Einträge stornieren (wo TC = Werber)
-- ============================================================================

-- Storno-Einträge für alle teamleitung-Buchungen wo der TC selbst der Werber ist
...
SELECT
    ...
    pl.year,
    pl.referenz_datum,
    'Migration 029: TC=Werber Korrektur',
    pl.campaign_id,
    pl.campaign_area_id,
    pl.customer_id
FROM provisions_ledger pl
JOIN records r ON r.id = pl.record_id
WHERE pl.kategorie = 'teamleitung'
  AND pl.typ = 'provision'
  AND r.werber_id = r.teamchef_id
  AND r.werber_id = pl.user_id;
